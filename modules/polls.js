        ...
        if (studentsAllowedToVote) {
            ...
            for (const student of classInformation.classrooms[classId].students) {
                ...
                if (
                    ...
                    student.break ||
                    student.tags.includes("Offline") ||
                    student.tags.includes("Excluded")
                ) {
                    continue;
                }
...
async function endPoll(classId, userSession) {
    try {
        ...
        await new Promise((resolve, reject) => {
            database.run("INSERT INTO poll_history(class, data, date) VALUES(?, ?, ?)", [classId, JSON.stringify(data), date], (err) => {
                if (err) {
                    logger.log("error", err.stack);
        ...
        let latestPoll = await new Promise((resolve, reject) => {
            database.get("SELECT * FROM poll_history WHERE class=? ORDER BY id DESC LIMIT 1", [classId], (err, poll) => {
                if (err) {
                    logger.log("error", err.stack);
                    reject(new Error(err));
                } else resolve(poll);
        ...
        for (const student of Object.values(classInformation.classrooms[classId].students)) {
            if (student.classPermissions < MANAGER_PERMISSIONS) {
                ...
                for (let i = 0; i < student.pollRes.buttonRes.length; i++) {
                    ...
                    database.run(
                        "INSERT INTO poll_answers(pollId, userId, buttonResponse) VALUES(?, ?, ?)",
                ...
                const studentId = student.id;
                database.run(
                    "INSERT INTO poll_answers(pollId, userId, textResponse) VALUES(?, ?, ?)",
