    ...
    try {
        ...
        if (classUser) {
            ...
            if (userSocketUpdates[email]) {
                userSocketUpdates[email].classUpdate(classroom.id, {
                    restrictToControlPanel: true,
                });
            }
            ...
            if (!user.isGuest) {
                await new Promise((resolve, reject) => {
                    database.run(
                        "INSERT INTO classusers(classId, studentId, permissions, digiPogs) VALUES(?, ?, ?, ?)",
                        [classroom.id, user.id, classInformation.classrooms[classroom.id].permissions.userDefaults, 0],
                        (err) => {
                            if (err) {
                                reject(err);
                                return;
