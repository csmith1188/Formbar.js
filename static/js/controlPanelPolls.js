...
function responseAmountChange(responseAmount = null) {
    ...
    for (let i = pollResponses.length; i < resNumber.value; i++) {
        ...
        FloatingUIDOM.autoUpdate(colorPickerButton, colorPickerDiv, () => {
...
function sharePoll(type) {
    if (type == "user") socket.emit("sharePollToUser", currentSharePollId, sharePollUserInput.value);
    else if (type == "class") socket.emit("sharePollToClass", currentSharePollId, sharePollClassInput.value);
...
function insertCustomPolls(customPollsList, customPollsDiv, emptyText) {
    ...
    for (let customPollId of customPollsList) {
        ...
        if (customPoll.name) customPollName.textContent = customPoll.name;
        else if (customPoll.prompt) customPollName.textContent = customPoll.prompt;
        else {
            unnamedPolls++;
...
document.addEventListener("click", (event) => {
    if (!event.target.closest(".colorPicker") && !event.target.classList.contains("colorPickerButton")) {
        let colorPickersDiv = document.getElementsByClassName("colorPicker");
    ...
    if (!event.target.closest("details.controlStudent") && !event.target.classList.contains("controlStudent")) {
