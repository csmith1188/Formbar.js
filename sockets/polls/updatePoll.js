...
module.exports = {
    run(socket, socketUpdates) {
        ...
        socket.on('updatePoll', async (options) => {
            try {
                logger.log('info', `[updatePoll] ip=(${socket.handshake.address}) session=(${JSON.stringify(socket.request.session)}) options=(${JSON.stringify(options)})`);
                ...
                const success = await updatePoll(classId, options, socket.request.session);
                if (success) {
