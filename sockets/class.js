    ...
    run(socket, socketUpdates) {
        ...
        socket.on("getCanVote", async (userId) => {
            try {
                logger.log(
                    "info",
                    `[getCanVote] userId=(${userId}) ip=(${socket.handshake.address}) session=(${JSON.stringify(socket.request.session)})`
                );
        ...
        socket.on("changeCanVote", async (votingData) => {
            try {
                logger.log(
                    "info",
                    `[changeCanVote] votingData=(${JSON.stringify(votingData)}) ip=(${socket.handshake.address}) session=(${JSON.stringify(socket.request.session)})`
                );
                ...
                for (const userId in votingData) {
                    ...
                    if (votingRight === true && studentsAllowedToVote.includes(userId) === false) {
                        ...
                        studentsAllowedToVote.splice(
                            0,
                            studentsAllowedToVote.length,
                            ...studentsAllowedToVote.filter((student) => student !== userId)
                        );
                    }
