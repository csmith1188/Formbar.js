    ...
    run(socket) {
        socket.on("poolCreate", async (data) => {
            try {
                ...
                if (typeof name !== "string" || name.length <= 0 || name.length > 50) {
                    return socket.emit("poolCreateResponse", {
                        success: false,
                ...
                if (member) {
                    dbRun("UPDATE digipog_pool_users SET owner = ? WHERE id = ?", [
                        member.owner ? member.owner + `,${result}` : `${result}`,
        ...
        socket.on("poolDelete", async (data) => {
            try {
                ...
                if (typeof poolId !== "number" || poolId <= 0) {
                    return socket.emit("poolDeleteResponse", {
                        success: false,
                ...
                const newOwnerList = member.owner
                    .split(",")
                    .filter((id) => id !== poolId.toString())
                    .join(",");
                await dbRun("UPDATE digipog_pool_users SET owner = ? WHERE id = ?", [newOwnerList, socket.request.session.userId]);
                await dbRun("UPDATE digipog_pool_users SET member = TRIM(REPLACE(REPLACE(member, ?, ''), ',,', ',')) WHERE member LIKE ?", [
                    poolId.toString(),
                    `%${poolId}%`,
                ]);
        ...
        socket.on("poolAddMember", async (data) => {
            try {
                ...
                if (typeof poolId !== "number") {
                    return socket.emit("poolAddMemberResponse", {
                        success: false,
                        message: "Invalid pool ID.",
                    });
                }
        ...
        socket.on("poolRemoveMember", async (data) => {
            try {
                const { poolId, userId } = data;
                if (typeof poolId !== "number") {
                    return socket.emit("poolRemoveMemberResponse", {
                        success: false,
                        message: "Invalid pool ID.",
                    });
                }
        ...
        socket.on("poolPayout", async (data) => {
            try {
                const { poolId } = data;
                if (typeof poolId !== "number" || poolId < 0) {
                    return socket.emit("poolPayoutResponse", {
                        success: false,
                        message: "Invalid pool ID.",
                    });
                }
                ...
                const members = await dbGetAll("SELECT id FROM digipog_pool_users WHERE member LIKE ? OR owner LIKE ?", [
                    `%${poolId}%`,
                    `%${poolId}%`,
                ]);
                for (const member of members) {
                    ...
                    if (user) {
                        ...
                        await dbRun("INSERT INTO transactions (from_user, to_user, pool, amount, reason, date) VALUES (?, ?, ?, ?, ?, ?)", [
                            ...
                            member.id,
                            pool.id,
                            Math.floor(pool.amount / members.length),
                            `Pool Payout`,
                            Date.now(),
                        ]);
                    }
