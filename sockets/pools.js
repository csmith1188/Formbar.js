...
module.exports = {
    run(socket) {
        socket.on("poolCreate", async (data) => {
            ...
            if (member) dbRun("UPDATE digipog_pool_users SET owner = ? WHERE user_id = ?", [member.owner.append(`,${result.lastID}`), socket.request.session.user.id]);
            else dbRun("INSERT INTO digipog_pool_users (user_id, owner) VALUES (?, ?)", [socket.request.session.user.id, result.lastID]);
